...
for time_bin in time_bins:
    ...
    if not crime_filtered.empty:
        ...
        crime_data_by_bin[time_bin] = crime_filtered["Weighted Severity Level"].values
    else:
        crime_trees[time_bin] = None
        crime_data_by_bin[time_bin] = None

# Function to compute severity level for an edge
def compute_severity(road_lat, road_lon, time_bin):
    """ Computes severity level for a given road midpoint and time bin """
    ...
    # Get severity values for these indices
    severity_values = crime_data_by_bin[time_bin][idx]

    # Compute mode of severity levels
    mode_values = pd.Series(severity_values).mode()
    
    return int(max(mode_values)) if not mode_values.empty else 0

# Compute severity levels for all time bins (Optimized)
for time_bin in time_bins:
    print(f"Processing severity for time bin: {time_bin}...")
    ...
    for edge in gdf_edges.itertuples(index=False):
        ...
        severity_level = compute_severity(road_lat, road_lon, time_bin)
        severities.append(severity_level)

    gdf_edges[f"severity_{time_bin}"] = severities  # Assign severity values efficiently
...
#Save the graph
ox.save_graphml(G_precomputed, "los_angeles_precomputed_severity.graphml")

print("✅ Preprocessing complete! Graph saved as 'los_angeles_precomputed_severity.graphml'.")
